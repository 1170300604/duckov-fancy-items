...
namespace FancyItems.Patches
{
    ...
    public static class ItemDisplayPatches
    {
        ...
        {
            var displays = Object.FindObjectsOfType<ItemDisplay>();
            var processed = 0;
        ...
        public static int CleanupAllQualityVisualizers()
        {
            ...
            // 清理视觉效果组件
            var visualizers = Object.FindObjectsOfType<Systems.UI.ItemQualityVisualizer>();
    ...
    public static class LootTimePatches
    {
        ...
        public static class LootingDataMGetInspectingTimePatch
        {
            ...
            {
                if (!Core.ModSetting.EnableSearchOptimization) return;

                // 在原始方法执行后，获取原始结果
                var originalTime = __result;

                // 使用我们的优化时间倍率计算
                var optimizedTimeScale = Systems.Optimization.SearchTimeOptimizer.GetOptimizedInspectingTimeScale(item);
